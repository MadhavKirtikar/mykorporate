...
const CalenderEvents = ({ user }) => {
  ...
  return (
    <div className="max-w-3xl mx-auto mt-10 px-2 sm:px-4 md:px-0">
      ...
      {newEvent && (
        <div className="mb-6 p-4 rounded-xl bg-green-50 border border-green-200 flex items-center gap-4 shadow">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-2">
            ...
            <span className="text-green-700">- {newEvent.description}</span>
          </div>
        </div>
      )}
          ...
          <tbody>
            {sortedEvents.length > 0 ? (
              sortedEvents.map((event, idx) => (
                <tr
                  key={idx}
                  className={`border-b last:border-b-0 transition hover:scale-[1.02] hover:shadow-lg hover:bg-purple-50 ${
                ...
                >
                  <td className="py-3 px-6 font-semibold text-blue-700 whitespace-nowrap">
                    <span className="inline-block px-3 py-1 rounded-full font-bold bg-gradient-to-r from-blue-400 via-purple-400 to-pink-400 text-white shadow border border-blue-200">
                  ...
                  <td className="py-3 px-6 font-bold text-purple-700">
                    {event.title}
                  </td>
              ...
              <tr>
                <td
                  colSpan={3}
                  className="py-8 text-center text-gray-400 text-lg font-medium bg-purple-50/40"
                >
