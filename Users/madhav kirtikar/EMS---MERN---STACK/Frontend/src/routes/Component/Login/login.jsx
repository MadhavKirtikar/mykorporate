...
const LoginRegister = () => {
  ...
  const { login } = useAuth(); // 👈 inside LoginRegister component
  ...
  const handleEmpLogin = async (e) => {
    ...
    try {
      ...
      login({ name: empEmail, role: "employee" }); // 👈 added
      if (empRemember) {
  ...
  const handleAdminLogin = async (e) => {
    ...
    try {
      ...
      localStorage.setItem('token', res.data.token);
      login({ name: adminUsername, role: "admin" }); // 👈 added
      if (adminRemember) {
  ...
  const handleAdminRegister = async (e) => {
    ...
    try {
      ...
      localStorage.setItem('token', res.data.token);
      login({ name: registerUsername, role: "admin" }); // 👈 added
