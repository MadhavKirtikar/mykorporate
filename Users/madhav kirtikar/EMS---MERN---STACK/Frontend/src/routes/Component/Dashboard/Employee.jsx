    ...
    <div className="flex min-h-screen bg-gradient-to-br from-purple-200 via-blue-100 to-white">
      ...
      >
        <div className="max-w-3xl mx-auto">
          ...
          {showAddForm && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
              <div className="flex w-full h-full items-center justify-center">
                <form
                  className="bg-white/95 backdrop-blur-lg rounded-2xl shadow-2xl p-4 md:p-6 w-full max-w-md mx-auto flex flex-col gap-3 relative border border-purple-100"
                  onSubmit={editMode ? handleUpdate : handleAdd}
                  style={{ zIndex: 100, marginLeft: "0", marginRight: "0", maxHeight: "90vh", overflowY: "auto" }}
                >
                  <h2 className="text-2xl font-extrabold text-transparent bg-gradient-to-r from-purple-600 via-blue-400 to-pink-400 bg-clip-text mb-3 text-center drop-shadow-lg tracking-wide">{editMode ? "Edit Employee" : "Add Employee"}</h2>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                    <input type="text" name="empId" value={editMode ? form.empId : generateEmpId(employees)} className="border border-purple-100 rounded-lg px-3 py-2 text-sm bg-white/80 text-gray-500 col-span-2 focus:ring-2 focus:ring-purple-200 transition" disabled readOnly />
                    <input type="text" name="name" placeholder="Name" value={form.name} onChange={handleChange} className="border border-purple-100 rounded-lg px-3 py-2 text-sm bg-white/80 focus:ring-2 focus:ring-purple-200 transition" required />
                    <select name="gender" value={form.gender} onChange={handleChange} className="border border-purple-100 rounded-lg px-3 py-2 text-sm bg-white/80 focus:ring-2 focus:ring-purple-200 transition" required>
                      <option value="">Gender</option>
                      <option value="Male">Male</option>
                      <option value="Female">Female</option>
                      <option value="Other">Other</option>
                    </select>
                    <input type="number" name="age" placeholder="Age" value={form.age} onChange={handleChange} className="border border-purple-100 rounded-lg px-3 py-2 text-sm bg-white/80 focus:ring-2 focus:ring-purple-200 transition" min={18} max={70} required />
                    <select name="department" value={form.department} onChange={handleChange} className="border border-purple-100 rounded-lg px-3 py-2 text-sm bg-white/80 focus:ring-2 focus:ring-purple-200 transition" required disabled={departmentsList.length === 0}>
                      <option value="">Department</option>
                      {departmentsList.map((dept, idx) => (
                        <option key={dept.id || dept.name + idx} value={dept.name}>{dept.name}</option>
                      ))}
                    </select>
                    <input type="text" name="position" placeholder="Position" value={form.position} onChange={handleChange} className="border border-purple-100 rounded-lg px-3 py-2 text-sm bg-white/80 focus:ring-2 focus:ring-purple-200 transition" />
                    <input type="number" name="salary" placeholder="Salary" value={form.salary} onChange={handleChange} className="border border-purple-100 rounded-lg px-3 py-2 text-sm bg-white/80 focus:ring-2 focus:ring-purple-200 transition" />
                    <input type="email" name="email" placeholder="Email" value={form.email} onChange={handleChange} className="border border-purple-100 rounded-lg px-3 py-2 text-sm bg-white/80 focus:ring-2 focus:ring-purple-200 transition" />
                    <input type="text" name="phone" placeholder="Phone" value={form.phone} onChange={handleChange} className="border border-purple-100 rounded-lg px-3 py-2 text-sm bg-white/80 focus:ring-2 focus:ring-purple-200 transition" />
                    <input type="text" name="address" placeholder="Address" value={form.address} onChange={handleChange} className="border border-purple-100 rounded-lg px-3 py-2 text-sm bg-white/80 focus:ring-2 focus:ring-purple-200 transition" />
                    <input type="text" name="photo" placeholder="Photo URL" value={form.photo} onChange={handleChange} className="border border-purple-100 rounded-lg px-3 py-2 text-sm bg-white/80 focus:ring-2 focus:ring-purple-200 transition" />
                    <div className="relative col-span-2">
                      <input type={showPassword ? "text" : "password"} name="password" placeholder="Password" value={form.password} onChange={handleChange} className="border border-purple-100 rounded-lg px-3 py-2 text-sm w-full pr-8 bg-white/80 focus:ring-2 focus:ring-purple-200 transition" required />
                      <button type="button" className={`absolute right-2 top-1/2 -translate-y-1/2 text-lg transition-colors duration-200 ${showPassword ? "text-purple-600" : "text-gray-400"} hover:text-purple-500`} onClick={() => setShowPassword((prev) => !prev)} tabIndex={-1} aria-label={showPassword ? "Hide password" : "Show password"} style={{ background: "none", border: "none", padding: 0, cursor: "pointer" }}>
                        {showPassword ? (
                          ...
                          <svg width="18" height="18" fill="none" viewBox="0 0 24 24"><path d="M17.94 17.94A10.97 10.97 0 0 1 12 19c-7 0-11-7-11-7a21.8 21.8 0 0 1 5.06-6.06M1 1l22 22" stroke="currentColor" strokeWidth="2"/></svg>
                        )}
                      </button>
                    </div>
                    {editMode && (
                      <input type="number" name="performance" placeholder="Performance (1-5)" value={editPerformance} onChange={(e) => setEditPerformance(e.target.value)} min={1} max={5} step={0.1} className="border border-purple-100 rounded-lg px-3 py-2 text-sm col-span-2 bg-white/80 focus:ring-2 focus:ring-purple-200 transition" />
                    )}
                  </div>
                  {addError && <div className="text-red-500 text-xs text-center mt-1">{addError}</div>}
                  <div className="flex gap-2 justify-end mt-2">
                    <button type="button" className="px-4 py-2 bg-gray-100 rounded-full hover:bg-gray-200 text-sm font-semibold shadow" onClick={() => { setShowAddForm(false); setForm(emptyForm); setEditMode(false); setSelectedEmp(null); setShowPassword(false); setEditPerformance(""); }}>Cancel</button>
                    <button type="submit" className="px-4 py-2 bg-gradient-to-r from-purple-500 to-blue-400 text-white rounded-full hover:from-purple-600 hover:to-blue-500 text-sm font-semibold shadow">{editMode ? "Update" : "Add"}</button>
                  </div>
                </form>
              </div>
            </div>
          ...
          {/* Modern summary cards with glassmorphism and gradient borders */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
            <div className="bg-white/60 backdrop-blur-lg rounded-2xl shadow-2xl p-5 text-center border-2 border-purple-200 hover:scale-105 transition-transform duration-300">
              <div className="text-3xl font-extrabold text-purple-700 drop-shadow-lg">{totalEmployees}</div>
              <div className="text-gray-600 font-medium mt-2">Total Employees</div>
            </div>
            <div className="bg-white/60 backdrop-blur-lg rounded-2xl shadow-2xl p-5 text-center border-2 border-blue-200 hover:scale-105 transition-transform duration-300">
              <div className="text-3xl font-extrabold text-blue-700 drop-shadow-lg">{totalDepartments}</div>
              <div className="text-gray-600 font-medium mt-2">Departments</div>
            </div>
            <div className="bg-white/60 backdrop-blur-lg rounded-2xl shadow-2xl p-5 text-center border-2 border-green-200 hover:scale-105 transition-transform duration-300">
              <div className="text-3xl font-extrabold text-green-700 drop-shadow-lg">₹{totalSalary}</div>
              <div className="text-gray-600 font-medium mt-2">Total Salary</div>
            </div>
          </div>
          {!viewEmp && (
            <>
